...
export async function main(payload: string, repo_url_resource_path: string, skip_secret: boolean) {
  ...
  if (repo_resource.branch === gh_payload.ref.split("/").pop()) {
    console.log("Matching branch, executing sync...")
    ...
    wmill_sync_push(process.env["WM_WORKSPACE"], skip_secret)
  }
  return gh_payload
}
...
async function wmill_sync_push(
  ...
  // --skip-secrets 
  // --skip-resources 
  // --workspace ${{ env.WMILL_WORKSPACE }} 
  // --token ${{ secrets.WMILL_TOKEN }} 
  // --base-url ${{ env.WMILL_URL }}
  await wmill_run(
    3,
    "sync",
    "push",
