 ...
 *
 * This is the placeholder step. Replace the body of `main` with a real call to whichever
 * system owns your group membership — an internal IdP, an LDAP bridge, an HR system, any
 * HTTP-reachable directory. Everything downstream only depends on the SHAPE of the return
 * value, so as long as you return the same structure, step 2 needs no changes.
 */
...
export async function main(): Promise<SourceGroup[]> {
  // ---------------------------------------------------------------------------
  // REPLACE EVERYTHING BELOW WITH YOUR DIRECTORY CALL.
  //
  // Sketch of what that looks like:
  //
  //   import * as wmill from "windmill-client"
  //
  //   const apiKey = await wmill.getVariable("u/admin/directory_api_key")
  //   const res = await fetch("https://directory.internal/api/v1/groups?app=windmill", {
  //     headers: { Authorization: `Bearer ${apiKey}`, Accept: "application/json" },
  //   })
  //   if (!res.ok) {
  //     throw new Error(`Directory returned ${res.status}: ${await res.text()}`)
  //   }
  ...
  return [
    ...
    },
    {
      ...
      summary: "Support team — operators",
